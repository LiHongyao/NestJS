...
> **！注意**：<DECORATOR_NAME> 可以含有路径，如：common/decorators/user，这样就会在 `src` 目录下建立该路径并含有 Decorator。

这里我创建了一个 `User` 装饰器：

```shell
$ nest g d common/decorators/user 
```
...
```shell
$ nest g mi common/middlewares/add-user
```
...
import { AddUserMiddleware } from './common/middlewares/add-user.middleware';
@Module({
...
```shell
$ nest g d common/decorators/auth
```
